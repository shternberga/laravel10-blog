<div class="bg-white dark:bg-slate-800 shadow-md rounded border border-slate-200 dark:border-slate-700 p-5">
    ...
    <form method="POST" action="{{ $isEdit ? route('posts.update', $post) : route('posts.store') }}" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            <input id="title-input" class="form-input w-full bg-slate-100 dark:bg-slate-900 border-transparent dark:border-transparent focus:bg-white dark:focus:bg-slate-800 placeholder-slate-500" type="text" name="title" placeholder="Your post title..." value="{{ old('title', $post->title ?? 'Your post title...') }}">
            @error('title')
            <div>{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <textarea id="body-input" class="form-input w-full bg-slate-100 dark:bg-slate-900 border-transparent dark:border-transparent focus:bg-white dark:focus:bg-slate-800 placeholder-slate-500" name="body" placeholder="Your new post content...">{{ old('body', $post->body ?? 'Your new post content') }}</textarea>
            @error('body')
            <div>{{ $message }}</div>
            @enderror
        </div>
